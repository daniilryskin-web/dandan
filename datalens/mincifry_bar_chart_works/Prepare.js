// ============================================================================
//  Вкладка Prepare — Advanced-чарт: горизонтальный стек по типам
//  со стадиями работ и суммой в конце полосы.
//
...
// aliases — запасные имена: те же меры встречаются и без суффикса
// «(индикатор)». Берём то имя, которое реально пришло.
// label — подпись в легенде: суффикс там не нужен.
const SERIES_SPEC = [
    {field: '0. Данные уточняются (индикатор)', aliases: ['0. Данные уточняются'],
     label: '0. Данные уточняются', color: BRAND.violet},
    {field: '1. Работы не начаты (индикатор)', aliases: ['1. Работы не начаты'],
     label: '1. Работы не начаты', color: BRAND.violetLight},
    {field: '2. В работе (индикатор)', aliases: ['2. В работе'],
     label: '2. В работе', color: BRAND.violetDeep},
    {field: '3. Реализовано (индикатор)', aliases: ['3. Реализовано'],
     label: '3. Реализовано', color: BRAND.magenta},
];
...
const TITLE = 'Стадии работ по типам';

// Порядок категорий сверху вниз. Заданный список надёжнее сортировки:
// «ВСЕГО» — итоговая строка, и по алфавиту она бы никуда не встала правильно.
// Категории, которых в списке нет, идут следом в порядке источника.
const CATEGORY_ORDER = ['ВСЕГО', 'Ведомство', 'ГК ИЭП', 'Гостех'];

// В подписях категорий тут нет приклеенного итога, отсекать нечего.
const STRIP_TOTAL_SUFFIX = false;
...
categories.sort(function (a, b) {
    ...
    if (ia === -1 && ib === -1) return 0;   // обе вне списка — порядок источника
    if (ia === -1) return 1;
    if (ib === -1) return -1;
    return ia - ib;
});
...
present.forEach(function (field) {
    if (resolved.filter(function (r) { return r.field === field; }).length) return;
    resolved.push({
        field: field,
        name: field,
...
const totals = categories.map(function (key) {
    return resolved.reduce(function (sum, r) {
        return sum + ((index[key] && index[key][r.field]) || 0);
    }, 0);
});
