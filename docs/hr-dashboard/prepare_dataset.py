...
def build_assignments(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["Граница выброса вверх"] = (df["Q3 роли"] + 1.5 * iqr).round(0)
    df["Граница выброса вниз"] = (df["Q1 роли"] - 1.5 * iqr).round(0)
    df["Выброс по IQR"] = "—"
    df.loc[in_scope, "Выброс по IQR"] = "В норме"
    df.loc[in_scope & (df["ЗП"] > df["Граница выброса вверх"]), "Выброс по IQR"] = (
        "Выброс вверх"
    )
    df.loc[in_scope & (df["ЗП"] < df["Граница выброса вниз"]), "Выброс по IQR"] = (
        "Выброс вниз"
    )
