...
# Декрет отмечается в столбце «Продукт»: проект у сотрудницы остаётся
# прежним («Контроль-надзор»), а вместо продукта пишется «Декрет».
# Принимаем любое написание, начинающееся с этого слова, — «Декретный
# отпуск» тоже сработает.
MATERNITY_MARKER = "декрет"
...
COLUMN_ORDER = [
    ...
    "Вакансия", "В декрете", "ЗП исходная", "ЗП", "ЗП подтверждена", "Комментарий",
...
def build_assignments(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # --- декрет --------------------------------------------------------------
    # Человек в декрете числится в штате, но работы за ним сейчас нет.
    # Поэтому он остаётся в реестре «Сотрудники» — чтобы сходилась общая
    # численность, — и выпадает из всего остального: из дерева структуры,
    # из статистики по ролям, из ФОТ и из анализа справедливости.
    #
    # Зарплату обнуляем принудительно, а не полагаемся на то, что в исходнике
    # стоит ноль: случайно оставленное число исказило бы и ФОТ, и медиану
    # роли, причём молча.
    maternity = pd.concat(
        [df["Продукт"], df["Продукт кратко"]], axis=1
    ).apply(
        lambda row: any(
            str(value).strip().lower().startswith(MATERNITY_MARKER)
            for value in row
        ),
        axis=1,
    )
    # Декрет — свойство человека, а не отдельной строки. Если у сотрудницы
    # кроме «Декрета» осталась строка на реальном продукте, она всё равно
    # в декрете целиком: иначе она попала бы в дерево одной строкой, а ЗП
    # обнулилась бы только на другой, и ФОТ разъехался бы с реестром.
    on_leave = (maternity & (df["Вакансия"] == 0))
    people_on_leave = set(df.loc[on_leave, "Сотрудник"]) - {""}
    df["В декрете"] = (
        (df["Вакансия"] == 0) & df["Сотрудник"].isin(people_on_leave)
    ).astype(int)
    df.loc[df["В декрете"] == 1, "ЗП"] = 0.0
    ...
    staffed = df[(df["Вакансия"] == 0) & (df["В декрете"] == 0)
                 & (df["Первичное назначение"] == 1)]
    ...
    product_people = (
        df[(df["Вакансия"] == 0) & (df["В декрете"] == 0)]
        .groupby("Продукт ключ")["Сотрудник"].nunique()
    )
...
def sheet_people(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Единственный лист, где остаются декретные: по нему сходится общая
    численность штата. Зарплата у них 0, поэтому в суммах они не участвуют,
    а отличить их можно по столбцу «Статус».
    """
    people = df[(df["Вакансия"] == 0) & (df["Первичное назначение"] == 1)].copy()
    people["Статус"] = people["В декрете"].map({1: "В декрете", 0: "Работает"})
    columns = [
        "Сотрудник", "Статус", "Проектная роль", "Уровень роли", "ЗП", "ЗП подтверждена",
...
def sheet_projects(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Декретные не считаются: работы за ними сейчас нет, а ФОТ у них нулевой.
    Общая численность штата — на листе «Сотрудники».
    """
    people = df[(df["Вакансия"] == 0) & (df["В декрете"] == 0)]
...
def sheet_hierarchy(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    # Декрет в дерево не попадает вообще: ни строкой «Декрет» в продуктах,
    # ни человеком в составе команды. Работы за этим человеком сейчас нет,
    # а численность штата целиком видна на листе «Сотрудники».
    active = df[df["В декрете"] == 0]
    people = active[active["Вакансия"] == 0]
    ...
    table = pd.DataFrame(index=active.groupby(index).size().index)
    ...
    if "Заместитель" in active.columns:
        deputy = (
            active.assign(_d=active["Заместитель"].fillna("").astype(str).str.strip())
            .groupby(index)["_d"]
    ...
    open_positions = active[active["Вакансия"] == 1]
    table["вакансий"] = (
...
def sheet_checks(df: pd.DataFrame) -> pd.DataFrame:
    ...
    checks = [
        ...
        ("В декрете (людей)",
         int(unique_people["В декрете"].sum()),
         "в штате и в реестре «Сотрудники»; в дереве, ФОТ и ролях не участвуют"),
        ("Работающих", int((unique_people["В декрете"] == 0).sum()),
         "справочно: уникальных минус декрет"),
