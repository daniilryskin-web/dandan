...
def main():
    ...
    check("50k: вариантов капится 1800", m.scaled_max_queries(50000, 250) == 1800)
    check("явное значение уважается", m.scaled_max_queries(20000, 300) == 300)

    # v49: на больших лимитах словарь даёт МНОГО вариантов запросов для ВСЕХ
    # категорий (раньше у техники/электроники/посуды было ~25-55).
    _mq = m.scaled_max_queries(50000, 250)
    for dom, q, mn in [('clothing', 'детская одежда', 800), ('shoes', 'обувь', 400),
                       ('toys', 'игрушки', 300), ('electronics', 'электроника', 300),
                       ('appliances', 'бытовая техника', 400), ('kitchenware', 'посуда', 300),
                       ('home', 'дом', 300), ('kids_accessories', 'аксессуары', 400)]:
        n = len(m.generate_query_variants(q, dom, _mq))
        check(f"{dom}: вариантов >= {mn} (было мало)", n >= mn)
    # «взрослым» категориям НЕ навешиваем возрастные слова (нет «детских стиральных»)
    _appl = [x.lower() for x in m.generate_query_variants('бытовая техника', 'appliances', _mq)]
